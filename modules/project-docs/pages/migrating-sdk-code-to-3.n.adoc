...
* From NuGet (the most popular choice): `Install-Package CouchbaseNetClient -Version 3.4.15`
* Or download a NuGet package directly from https://www.nuget.org/packages/CouchbaseNetClient/3.4.15[https://www.nuget.org/packages/CouchbaseNetClient/3.4.15]
...
There are several dependency changes from SDK 2.x to 3.4.x.
...
Analytics querying, like {sqlpp}, is also moved to the `Cluster` level: it is now accessible through the `Cluster.AnalyticsQueryAsync` method.
