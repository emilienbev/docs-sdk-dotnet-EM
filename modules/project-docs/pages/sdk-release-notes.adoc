...
=== Version 3.6.2 (22 July 2024)

Version 3.6.2 is a hotfix releases.
Microsoft disclosed a CVE affecting their `System.Text.Json` (STJ) library on the 9th of July, 2024. 
This release upgrades STJ to the patched version in our SDK.

* https://github.com/advisories/GHSA-hh2w-p6rv-4g7w[Microsoft CVE on GitHub]

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.2.zip[Download] |
...
Vulnerability in `System.Text.Json` 8.0.0 is fixed.

==== Improvements

* https://issues.couchbase.com/browse/NCBC-3821[NCBC-3821]:
Updated `CouchbaseNetClientReleasedVerison` to 3.6.2.

* https://issues.couchbase.com/browse/NCBC-3726[NCBC-3726]: 
Applied some miscellaneous performance optimizations to transcoders.

* https://issues.couchbase.com/browse/NCBC-3795[NCBC-3795]: 
Reduced duplicate operation handling code in `ClusterNode`.


=== Version 3.6.1 (3 July 2024)

Version 3.6.1 is a hotfix release.

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.1.zip[Download] |
...
`BucketConfig` version used to be compared against the latest applied config, not the latest one received.

* https://issues.couchbase.com/browse/NCBC-3698[NCBC-3698]:
Manually setting `ScopeName` and/or `CollectionName` in the `CollectionQueryIndexManager` API's options will now throw an `InvalidArgumentException`.
Previously, the SDK was ignoring the `ScopeName` and `CollectionName` in options, instead of throwing `InvalidArgumentException` as per RFC.

* https://issues.couchbase.com/browse/NCBC-3780[NCBC-3780]:
Transactions: `QueryContext` is now copied when retrying `SingleQueryTransactions`.

==== Improvement

* https://issues.couchbase.com/browse/NCBC-3798[NCBC-3798]: VectorSearch: Passing in empty vectors now throws InvalidArgumentException


=== Version 3.6.0 (13 June 2024)

Version 3.6.0 is the first release of the 3.6 series.

https://packages.couchbase.com/clients/net/3.6/Couchbase-Net-Client-3.6.0.zip[Download] |
https://docs.couchbase.com/sdk-api/couchbase-net-client-3.6.0[API Reference] |
https://www.nuget.org/packages/CouchbaseNetClient/3.6.0[Nuget]
...
* https://issues.couchbase.com/browse/NCBC-3572[NCBC-3572]:
For management services, `CancellationTokens` are now more accurate to the operation's lifetime.
* https://issues.couchbase.com/browse/NCBC-3702[NCBC-3702]:
Stellar client: Use `DefaultSerializer` as default at Cluster level 
* https://issues.couchbase.com/browse/NCBC-3760[NCBC-3760]:
Fixed an issue regarding passing an `X509CertificateFactory` in `ClusterOptions`.
* https://issues.couchbase.com/browse/NCBC-3768[NCBC-3768]:
Aligned Boolean search API to RFC.
* https://issues.couchbase.com/browse/NCBC-3775[NCBC-3775]:
`IndexAlreadyExists` is now properly thrown.
* https://issues.couchbase.com/browse/NCBC-3785[NCBC-3785]:
Transactions: `DocumentMetadata` deserialization fixed.
* https://issues.couchbase.com/browse/NCBC-3789[NCBC-3789]:
Updated Release version tag in SDK `Directory.Build.Props`.
* https://issues.couchbase.com/browse/NCBC-3794[NCBC-3794]:
Removed infinite loop in `ConfigPushHandler`, which was causing high CPU usage.

==== New Feature

* https://issues.couchbase.com/browse/NCBC-3788[NCBC-3788]:
Support addid for base64 encoded vector types.

==== Improvements

* https://issues.couchbase.com/browse/NCBC-3771[NCBC-3771]:
Refactored `RetryOrcestrator` for Data Service operations to be clearer.
* https://issues.couchbase.com/browse/NCBC-3792[NCBC-3792]:
Optimize for larger InFlightOperationSet for high latency connections.
* https://issues.couchbase.com/browse/NCBC-3796[NCBC-3796]:
Updated local build version to 3.6.0.
* https://issues.couchbase.com/browse/NCBC-3797[NCBC-3797]:
Updated `VectorSearch`'s InterfaceStability to Committed.
...
* https://issues.couchbase.com/browse/NCBC-3811[NCBC-3811]: Backports ConfigPushHandler fixes to 3.5 branch
(See also https://issues.couchbase.com/browse/NCBC-3791[NCBC-3791], https://issues.couchbase.com/browse/NCBC-3794[NCBC-3794])
...
https://packages.couchbase.com/clients/net/3.5/Couchbase-Net-Client-3.5.2.zip[Download] |
...
NOTE: Owing to https://issues.couchbase.com/browse/NCBC-3794[NCBC-3794], it is recommended that all customers using Server 7.6 or later upgrade to SDK 3.6.1 or later immediately.


==== Fixed Issues

* https://issues.couchbase.com/browse/NCBC-3745[NCBC-3745]:
`MultiplexingConnection` will throw a `SocketNotAvailableException` if disposed.
* https://issues.couchbase.com/browse/NCBC-3756[NCBC-3756]:
Handle empty `VBucketMap` in server config.
* https://issues.couchbase.com/browse/NCBC-3758[NCBC-3758]:
Fixed regression where `PersistentQueue` was not dequeueing items in original FIFO order.
* https://issues.couchbase.com/browse/NCBC-3764[NCBC-3764]:
Fixed issue where `GetAnyReplica` / `TaskHelpers.WhenAnySuccessful` could potentially cause deadlocks using the .NET Framework.
* https://issues.couchbase.com/browse/NCBC-3769[NCBC-3769]:
SDK will now adapt to new cluster topology if only a port has changed (as in some Alternate Address configurations).
* https://issues.couchbase.com/browse/NCBC-3772[NCBC-3772]:
Fixed a regression where configs were being applied to buckets they did not belong to, including cluster-level config.

==== New Features and Behavioral Changes

* https://issues.couchbase.com/browse/NCBC-1622[NCBC-1622]:
Snappy Compression -- using the Snappier library -- is now merged into the main SDK.
* https://issues.couchbase.com/browse/NCBC-3738[NCBC-3738]:
Support added for scoped eventing functions.
* https://issues.couchbase.com/browse/NCBC-3746[NCBC-3746]:
Performance optimizations added to `ArrayExtensions`.
* https://issues.couchbase.com/browse/NCBC-3759[NCBC-3759]:
`ExceptionDispatchInfo` was being used to capture and rethrow the `Exception` stack trace at the time the exception was originally caught.
`ClusterContext` bootstrap is now used to track per-node bootstrap errors, using `ExceptionDispatchInfo` instead of `Exception`. 
This may result in more accurate stack traces now or in future runtimes.
* https://issues.couchbase.com/browse/NCBC-3770[NCBC-3770]:
Reduced the amount of `DEBUG` logging around Config Push -- the additional information is now logged at `TRACE` level.
